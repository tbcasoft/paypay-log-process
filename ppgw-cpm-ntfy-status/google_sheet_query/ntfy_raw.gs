function getRawNtfyStatus() {
  ...
  var stmt = conn.prepareStatement(`
      SELECT mhqr.time as Time, mhqr.merchantName as MerchantName, mhqr.value as Value, mhqr.otcRequestId as OtcRequestId, 
      mhqr.resultCode as ResultCode 
      FROM hour_qr.mainnet_ntfy_status_raw mhqr  
      WHERE time >= ? AND time < ?
      AND resultCode LIKE ?
      ORDER BY time 
    `);
  ...
  var resultCode = raw_data_sheet.getRange("F2").getValue();

  switch(resultCode) {
    case "HIVEX_ERROR":
      resultCode = "%ERROR%"
      break;
    case "HIVEX_UNAVAILABLE_MERCHANT":
      resultCode = "%UNAVAILABLE_MERCHANT%"
      break;
    default:
      resultCode = "%HIVEX%"
  }
...
function clearRange(sheet){
    ...
    var range = sheet.getRange(6, 2, rowCount, 5);
    range.clear();
}
